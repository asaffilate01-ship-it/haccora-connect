  ...
  return (
    <div className="p-6 md:p-10 space-y-8">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <div className="eyebrow">{"Kitchen & quality"}</div>
          <h1 className="mt-1 text-3xl md:text-4xl">{"Frying oil quality (TPM)"}</h1>
          <p className="text-muted-foreground mt-1 max-w-2xl">
            {
              "Trend total polar material (TPM) against the oil manufacturer's discard limit and your documented site procedure."
            }
          </p>
        </div>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <Kpi label={"Change soon"} value={changeSoon} tone="warn" />
        <Kpi label={"Over limit"} value={rejected} tone="danger" />
      ...
      <div className="surface overflow-hidden">
        {loading ? (
          ...
          <div className="p-10 text-center text-sm text-muted-foreground">{"No tests yet."}</div>
        ) : (
              ...
              <li key={i.id} className="p-5 flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-muted-foreground mt-1 flex flex-wrap gap-x-4 gap-y-1">
                    ...
                    <span>{new Date(i.tested_at).toLocaleString("en-GB")}</span>
                  </div>
