...
test("expired entitlement, wrong country and disabled offers fail closed", () => {
  ...
  assert.equal(
    canOffer(offer, { active: true, expiresAt: "2000-01-01" }, "GB"),
    false,
  );
  assert.equal(
    canOffer(offer, { active: true, expiresAt: "2999-01-01" }, "DE"),
    false,
  );
  assert.equal(
    canOffer({ ...offer, enabled: false }, {
      active: true,
  ...
  assert.equal(
    canOffer(offer, { active: true, expiresAt: "2999-01-01" }, "GB"),
    true,
  );
});
test("checkout redirect origin is exact and requires HTTPS", () => {
  assert.equal(
    safeSessionUrl("https://connect.gigs.com/session/x", [
      "https://connect.gigs.com",
    ]),
