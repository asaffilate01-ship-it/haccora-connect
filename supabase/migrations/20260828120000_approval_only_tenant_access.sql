-- Approval-only tenant ownership, subscription-bound capacity and billing recovery.
--
-- Tenant owners are provisioned by a platform owner. A tenant may add people or
-- premises only while an approved trial or paid subscription is current. Existing
-- users retain access during a short failed-payment grace period, but capacity
-- changes stop immediately. No customer evidence is deleted by this lifecycle.
...
-- Preserve already-provisioned tenants as approved without inventing an approver.
-- New tenants always record the platform owner who approved them.
...
-- New owner workspaces cannot be created by a public or ordinary authenticated
-- account. Platform provisioning and accepted team invitations remain available.
