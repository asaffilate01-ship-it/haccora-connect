...
-- The client and invitation function added `equipment` in Phase 22. Replace
-- legacy unnamed checks so the database accepts exactly the same scope set.
do $$
declare
...
comment on function public.get_platform_overview() is
  'Returns audited aggregate service metrics without exposing tenant operational records.';
comment on function public.get_platform_customers() is
  'Returns audited customer-account and subscription metadata without exposing operational evidence or staff PII.';
